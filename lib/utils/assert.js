/*
 * -- Context --
 * Passing wrong arguments types to a function is a programmer error, and such errors must be thrown.
 ...
 * It is not using any assertion library in order to keep the package's dependency tree small. Yet, if this module becomes
...
module.exports = {
  ...
  instanceOf(obj, ConstructorFn, msg = '') {
    ...
    throwErr(msg, `Expected object to be instance of ${ConstructorFn.name}`);
  },
  ...
  isObjectOrString(objOrStr, msg = '') {
    if (
      typeof objOrStr === 'string' ||
      (objOrStr !== null && typeof objOrStr === 'object')
    )
      return;
    throwErr(msg, `Expected obj to be an object or a string`);
  },
