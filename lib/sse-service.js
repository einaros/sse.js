...
class SSEService extends EventEmitter {
  ...
  register(req, res) {
    ...
    const onHTTPResponseReady = err => {
      if (err) return emitErr(err, 'Could not register connection');
    ...
    let mayRegisterConnection;
    try {
      mayRegisterConnection = verifyRequest(req, res);
    } catch (e) {
      return emitErr(e, 'Could not register connection');
    }
    ...
    if (mayRegisterConnection) {
      applyAsyncLifecycleHooks('beforeRegister', [req, res], err => {
        ...
        internal(this).setupHTTPResponse(res, onHTTPResponseReady);
      });
    }
  }
 ...
 * @property {transformResponseHeadersFn} [transformResponseHeaders]
...
/**
 * @callback transformResponseHeadersFn
