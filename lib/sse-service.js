  ...
  register(req, res) {
    ...
    if (getSSEIdFromResponse(res, SSEID) === null) {
      ...
      try {
        ...
        res.on('close', () => this.unRegister(sseId, () => this.emit('clientClose', sseId, res.locals)));
      ...
      res.write(`:${HEARTBEAT_MESSAGE}\n\n`, (err) => {
        if (err) {
          this.emit('error', wrapError(err, 'Could not send initial heartbeat payload'));
          return;
        }
...
function applyForTarget(fn, target, cb) {
  ...
  target = typeof target === 'function' && target !== cb || target instanceof internal(this).SSEID ? target : null;
  
  const {activeSSEConnections, SSEID} = internal(this);
  
  if (target instanceof SSEID) {
    if (!target.isConnectionActive)
      return;
...
function validateRequestForSSE(req, res) {
  if (res.headersSent)
    return {status: 500, message: 'Cannot register a connection with headers already sent'};
  if (req.headers['accept'] !== 'text/event-stream')
    return {
      status: 400,
      message: 'Cannot register connection : The request HTTP header "Accept" is not set to "text/event-stream"'
    };
  return null;
}

function writeSSELocalsInResponse(req, res, sseId) {
  if (!(res.locals instanceof Object))
    res.locals = {};
  
  const sse = {};
  if (req.headers['last-event-id'])
    sse.lastEventId = req.headers['last-event-id'];
  Object.defineProperty(sse, 'sseId', {value: sseId, writable: false, enumerable: true, configurable: false});
  Object.defineProperty(res.locals, 'sse', {value: sse, writable: false, enumerable: true, configurable: false});
}
